...
plugins: libs
	$(MAKE) all -C plugins/ensemble-chorus
...
clean:
	...
	$(MAKE) clean -C plugins/ensemble-chorus
	$(MAKE) clean -C tools/layout-tool
	rm -rf bin build

install: all
	$(MAKE) install -C plugins/ensemble-chorus

install-user: all
	$(MAKE) install-user -C plugins/ensemble-chorus
